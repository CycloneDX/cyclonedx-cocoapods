...
RSpec.describe CycloneDX::CocoaPods::Pod do
  context 'when creating new pods' do
    ...
    context 'with an invalid name' do
      ...
      it 'should raise an error' do
        invalid_pod_names.each do |pod_name|
    ...
    context 'with a valid name' do
      let(:valid_pod_names) do
        %w[Alamofire FirebaseAnalytics R.swift Sentry Dèja%Vú Sentry/Core GoogleUtilities/NSData+zlib NSDate+TimeAgo]
      end
      let(:valid_pod_root_names) do
        %w[Alamofire FirebaseAnalytics R.swift Sentry Dèja%Vú Sentry GoogleUtilities NSDate+TimeAgo]
      end
      ...
      context 'and a valid version' do
        ...
        let(:base_purls) do
          %w[
            pkg:cocoapods/Alamofire@5.0 pkg:cocoapods/Alamofire@6.8.3
            pkg:cocoapods/Alamofire@2.2.0-alpha.372
            pkg:cocoapods/FirebaseAnalytics@5.0 pkg:cocoapods/FirebaseAnalytics@6.8.3
            pkg:cocoapods/FirebaseAnalytics@2.2.0-alpha.372
            pkg:cocoapods/R.swift@5.0 pkg:cocoapods/R.swift@6.8.3
            pkg:cocoapods/R.swift@2.2.0-alpha.372
            pkg:cocoapods/Sentry@5.0 pkg:cocoapods/Sentry@6.8.3
            pkg:cocoapods/Sentry@2.2.0-alpha.372
            pkg:cocoapods/D%C3%A8ja%25V%C3%BA@5.0 pkg:cocoapods/D%C3%A8ja%25V%C3%BA@6.8.3
            pkg:cocoapods/D%C3%A8ja%25V%C3%BA@2.2.0-alpha.372
            pkg:cocoapods/Sentry@5.0#Core pkg:cocoapods/Sentry@6.8.3#Core
            pkg:cocoapods/Sentry@2.2.0-alpha.372#Core
            pkg:cocoapods/GoogleUtilities@5.0#NSData%2Bzlib pkg:cocoapods/GoogleUtilities@6.8.3#NSData%2Bzlib
            pkg:cocoapods/GoogleUtilities@2.2.0-alpha.372#NSData%2Bzlib
            pkg:cocoapods/NSDate%2BTimeAgo@5.0 pkg:cocoapods/NSDate%2BTimeAgo@6.8.3
            pkg:cocoapods/NSDate%2BTimeAgo@2.2.0-alpha.372
          ]
        end
        ...
        shared_examples 'pod_with_checksum' do
          context 'without checksum' do
            let(:checksum) { nil }
            let(:valid_pods) do
              valid_pod_names_and_versions.map do |name, version|
                described_class.new(name: name, version: version, source: source)
              end
            end
          ...
          context 'with a valid checksum' do
            ...
            let(:valid_pods) do
              valid_pod_names_and_versions.map do |name, version|
                described_class.new(name: name, version: version, source: source, checksum: checksum)
              end
            end
            it_behaves_like 'valid_pod'
          end
          ...
          context 'with an invalid checksum' do
            it 'should raise an error' do
              expect do
                described_class.new(name: valid_pod_names.sample, version: valid_versions.sample,
        ...
        context 'with source' do
          context 'from a repository' do
            ...
            context 'from an alternative repository' do
              ...
              let(:expected_purls) do
                base_purls.map do |purl|
                  purl.insert(purl.index('#') || -1, "?repository_url=#{CGI.escape(source.url)}")
                end
              end
              it_behaves_like 'pod_with_checksum'
            end
          ...
          context 'from a git repository' do
            context 'with just a URL' do
              ...
              let(:expected_purls) do
                base_purls.map do |purl|
                  purl.insert(purl.index('#') || -1, "?vcs_url=#{CGI.escape(source.url)}")
                end
              end
              it_behaves_like 'pod_with_checksum'
            end
            ...
            context 'with a tag' do
              ...
              let(:expected_purls) do
                base_purls.map do |purl|
                  purl.insert(purl.index('#') || -1, "?vcs_url=#{CGI.escape(source.url)}%40#{CGI.escape(source.label)}")
                end
              end
              it_behaves_like 'pod_with_checksum'
            end
          ...
          context 'from local disk' do
            ...
            let(:expected_purls) do
              base_purls.map do |purl|
                purl.insert(purl.index('#') || -1, "?file_name=#{CGI.escape(source.path)}")
              end
            end
            it_behaves_like 'pod_with_checksum'
          end
          ...
          context 'from other location' do
            ...
            let(:expected_purls) do
              base_purls.map do |purl|
                purl.insert(purl.index('#') || -1, "?download_url=#{CGI.escape(source.url)}")
              end
            end
            it_behaves_like 'pod_with_checksum'
          end
